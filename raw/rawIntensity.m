...
if nfiles >= 1
    for i=1:nfiles
        ...
        p_one = polyfit(data(1:5,i), actual(1:5)', 1);
        p_two = polyfit(data(6:10,i), actual(6:10)', 1);
        p_average = [0,0];
        p_average(1) = (p_one(1)+p_two(1)) / 2;
        p_average(2) = (p_two(2)+p_two(2)) / 2;
        scaled_data(i) = p_average(1) * (data(11,i)-data(1,i)) + 6;
    end
end

xlswrite('scaled_data.xlsx', data);
xlswrite('scaled_data.xlsx', scaled_data, 1, 'A12');
